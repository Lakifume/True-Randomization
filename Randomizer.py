...
class Main(QWidget):
    ...
    def initUI(self):
        ...
        self.setLayout(grid)
        self.setStyleSheet("QWidget{color: #ffffff; font-family: Cambria; font-size: 18px}" + "QMessageBox{background-color: #21222e}" + "QPushButton{background-color: #21222e}" + "QLineEdit{background-color: #21222e}" + "QToolTip{border: 1px solid white; background-color: #21222e; color: #ffffff; font-family: Cambria; font-size: 18px}")
    ...
    def check_for_updates(self):
        if requests.get("https://api.github.com/repos/Lakifume/True-Randomization/releases/latest").json()["tag_name"] != config[13]["Value"]["Tag"]:
            choice = QMessageBox.question(self, "Auto Updater", "New version found, update ?", QMessageBox.Yes | QMessageBox.No)
